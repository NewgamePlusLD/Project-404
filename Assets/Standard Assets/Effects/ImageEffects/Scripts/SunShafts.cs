...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class SunShafts : PostEffectsBase
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination) {
            ...
            if (sunTransform)
				v = targetCam.WorldToViewportPoint (sunTransform.position);
            else
            ...
            if (!useDepthTexture) {
				var format= targetCam.allowHDR ? RenderTextureFormat.DefaultHDR: RenderTextureFormat.Default;
                RenderTexture tmpBuffer = RenderTexture.GetTemporary (source.width, source.height, 0, format);
                RenderTexture.active = tmpBuffer;
				GL.ClearWithSkybox (false, targetCam);
